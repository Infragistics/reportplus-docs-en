...
As with the Map View, the map type displayed can either be the standard political map, a satellite picture or a hybrid between the two. The Heat Map settings has additional settings to configure, including link:#HeatMapLayers[Layers], link:#GeoLocation[Geo Location] and link:#WeightHeatMap[Weight]. *Geocoding is not supported in the Heat Map visualization*.
